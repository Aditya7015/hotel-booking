...
export const createBooking = async (req, res) => {
  try {
    ...
    const newBooking = await Booking.create({
      ...
      checkOutDate: checkOut,
      totalPrice,
      paymentStatus: "pending", // 👈 ensure schema matches this
    });
    ...
    const mailOptions = {
      ...
      html: `
        ...
        <ul>
          ...
          <li><strong>Check-In:</strong> ${newBooking.checkInDate.toDateString()}</li>
          <li><strong>Check-Out:</strong> ${newBooking.checkOutDate.toDateString()}</li>
          <li><strong>Amount:</strong> ${process.env.CURRENCY || "$"} ${newBooking.totalPrice}</li>
        </ul>
      `
    };
...
export const stripePayment = async (req, res) => {
  try {
    ...
    const session = await stripeInstance.checkout.sessions.create({
      ...
      success_url: `${origin}/loader/my-bookings`,
      cancel_url: `${origin}/my-bookings`,
      metadata: { bookingId: booking._id.toString() }, // 👈 ensures webhook can update
    });
